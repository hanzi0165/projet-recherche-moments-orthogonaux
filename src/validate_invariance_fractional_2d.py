...
from fractional_2d import frcm_2d, normalized_coords_01
...
def flatten_moments(img, n_max, alpha_x=1.0, alpha_y=1.0):
    M, _, _ = frcm_2d(img, n_max, alpha_x=alpha_x, alpha_y=alpha_y)
    return M.flatten()
...
v0 = flatten_moments(img, n_max, alpha_x=alpha_x, alpha_y=alpha_y)
...
print("=== Fractional 2D Invariance / stabilité test ===")
print(f"n_max = {n_max}, alpha_x = {alpha_x}, alpha_y = {alpha_y}")
print()

for name, img_t in tests.items():
    vt = flatten_moments(img_t, n_max, alpha_x=alpha_x, alpha_y=alpha_y)
